...
float noise(float t){return texture(iChannel0,vec2(t,.0)/iChannelResolution.xy).x;}
float moy = 0.;
...
float noise(vec3 p)
{
	...
	vec2 rz = texture( iChannel0, (tap+0.5)/256.0 ).yx;
...
float path(float x){ return sin(x*0.01-3.1415)28.1+6.5; }
float map(vec3 p){
