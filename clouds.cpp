...
#define pixelsx 640
#define pixelsy 480
#define pixelsxw (1/640)
#define pixelsyw (1/480)
...
vector<float> relativedistance[640*480];
...
void CreateWorleyPointsBuffer () {
         ...
         newpoints.resize((numCellsPerAxis+2) *(numCellsPerAxis+2)*(numCellsPerAxis+2));
        ...
        for (int x = 0; x < numCellsPerAxis; x++) {
            for (int y = 0; y < numCellsPerAxis; y++) {
                for (int z = 0; z < numCellsPerAxis; z++) {
                    ...
                    // cout<<randomX<<" "<<randomY<<" "<<randomZ<<" "<<endl;
                    // float randomX = (float) rand();
                    // float randomY = (float) rand();
                    // float randomZ = (float) rand();
                    glm::vec3 randomOffset = cellSize*glm::vec3(randomX, randomY, randomZ);
                    glm::vec3 cellCorner = cellSize*glm::vec3 (x, y, z) ;
...
// cout<<glm::length(glm::vec3(0.5,0.0,0.0))<<endl;
